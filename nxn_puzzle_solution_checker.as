; program
; Check if an nxn puzzle has a solution
...
MANHATTAN:		PUSH 	R4					; Preserve the records
				...
				PUSH 	R6
				PUSH 	R7
				MOV 	R4, puzzle			; Defines the starting address of the puzzle
				MOV 	R6, 0				; Set the index to 0
			
SEARCHEMPTY:	MOV 	R5, M[R4]			; Read current memory address with puzzle element
				CMP 	R5, M[empty]		; Check if the square is empty
				...
				INC 	R6					; INCREASE the index
				CMP 	R6, M[sizing]
				BR.NN 	ERROR1				; Check if there is overflow in the puzzle
				JMP 	SEARCHEMPTY			; Loop to NEXT puzzle element
				
CALCULATE:		MOV 	R7, M[dimension]	; Definie a dimension do puzzle
				DIV 	R6, R7				; Set the size of the puzzle
				DIV R6, R7					; Divide the index with the dimension. (dimension - quotient - 1) + (dimension - remainder - 1) is the distance from Manhattan to the last position of the puzzle.
...
LOOP1:			CMP 	R6, M[sizing]		; Check if it is the END of the puzzle
				BR.Z 	OUT2
				MOV 	R7, 0
...
VALIDATION:   	PUSH 	R2					; Preserve the records
				...
				MOV 	R6, 0				; Index
				MOV 	R7, M[sizing]		; sizing of puzzle
				INC 	R7
...
NEXT:		SUB 	R6, R4
				...
				CMP 	R6, M[sizing]		; Check if it is the END of the puzzle
				...
				JMP 	LOOP3				; Loop with NEXT element
